...
st.set_page_config(
    page_title="Portal del Jugador | ADARVE DH",
    page_icon="⚡",
    layout="wide"
)
...
def inject_v0_css():
    st.markdown(f"""
        ...
        div.stButton > button {{
            ...
            color: {TEXT}; padding: 0.45rem 0.5rem; transition: all 0.2s; font-size: 0.85rem; font-weight: 700;
        }}
...
if not os.path.exists(_ruta_logo_sidebar):
    _ruta_logo_sidebar = os.path.abspath(os.path.join(_carpeta_pages, "..", "assets", "logo-guille_blanco.png"))
...
if not st.session_state['jugador_autenticado']:
    ...
    with col_c2:
        ...
        if st.button("ACCEDER A MI INFORME", type="primary", use_container_width=True):
            r_pos_check = os.path.join("data", "Posiciones.xlsx")
            if os.path.exists(r_pos_check):
                df_p_check = pd.read_excel(r_pos_check)
                col_pass = next((c for c in df_p_check.columns if 'pass' in str(c).lower() or 'clave' in str(c).lower() or 'pin' in str(c).lower()), None)
                col_nom = next((c for c in df_p_check.columns if 'jugador' in str(c).lower() or 'nombre' in str(c).lower()), df_p_check.columns[0])
                
                if col_pass:
                    df_p_check['PASS_STR'] = df_p_check[col_pass].astype(str).str.split('.').str[0].str.strip()
                    pin_str = str(pin_ingresado).strip()
                    
                    match = df_p_check[df_p_check['PASS_STR'] == pin_str]
                    if not match.empty:
                        st.session_state['jugador_autenticado'] = str(match.iloc[0][col_nom]).strip()
                        st.rerun()
                    else:
                        st.error("❌ PIN incorrecto. Revisa tu clave e inténtalo de nuevo.")
                else:
                    st.error("❌ No se encontró la columna 'PASSWORD' en Posiciones.xlsx.")
            else:
                st.error("❌ No se encontró el archivo de datos 'data/Posiciones.xlsx'.")
    st.stop()

# =============================================================================
# 3. CARGA DE DATOS MULTIFUENTE CON RANKING SINCRONIZADO
# =============================================================================
...
def cargar_todo_informes():
    ...
    if not df_pos.empty:
        ...
        c_foto = next((c for c in df_pos.columns if any(k in c.lower() for k in ['foto', 'url', 'fotos'])), None)
    ...
    if not df_calendario.empty:
        ...
        if c_f:
            ...
            for _, row in df_calendario.dropna(subset=['Fecha_Date']).iterrows():
                ...
                dict_calendario[f_date] = {'Rival': rival, 'Localizacion': loc, 'Escudo': escudo}
...
def get_logo(rival_str):
    ...
    for k, v in dict_escudos.items():
        if k in r_low or r_low in k: return v
    return "https://cdn-icons-png.flaticon.com/512/53/53283.png"

# =============================================================================
# 3. BLOQUEO / ASIGNACIÓN AUTOMÁTICA DEL JUGADOR LOGEADO
# =============================================================================
jugador_sel = st.session_state.get('jugador_autenticado', None)
if not jugador_sel:
    st.error("⚠️ Sesión expirada o no autenticada.")
    st.stop()

# BOTÓN DE CERRAR SESIÓN EN SIDEBAR
with st.sidebar:
    st.markdown(f"**👤 Jugador:** `{jugador_sel}`")
...
if not df_p_jug.empty:
    ...
    for idx, r in df_p_jug.iterrows():
        ...
        if r['Is_Partido']:
            ...
            labels.append(f"{riv} ({loc})" if loc else riv)
...
# CONDITIONING TESTS (RADAR DE 8 DIMENSIONES SIN LUMBAR)
with tab_tests:
    ...
    def get_best(df, col_jugador, jug_norm_val, col_val, filter_col=None, filter_val=None, exact=False):
        ...
        if filter_col and filter_col in dff.columns and filter_val: 
            if exact: dff = dff[dff[filter_col].astype(str).str.upper() == str(filter_val).upper()]
            else: dff = dff[dff[filter_col].astype(str).str.contains(filter_val, case=False, na=False)]
    ...
    with left_col:
        ...
        if test_categoria == "🩺 Movilidad":
            ...
            if not df_j_mov.empty:
                ...
                fig_m.add_hrect(y0=val_amarillo, y1=val_verde, fillcolor="rgba(245, 158, 11, 0.12)", line_width=0, layer="below")
            ...
            if not df_j_dina.empty:
                ...
                fig_d.update_layout(
                    height=400, barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color=TEXT),
                    xaxis=dict(tickangle=0, dividerwidth=2, dividercolor=BORDER), yaxis_title="Fmax Abs (N)",
            ...
            if not df_j_campo.empty:
                ...
                if ref_vmax and pd.notna(ref_vmax): fig_vel.add_hline(y=ref_vmax, line_dash="dash", line_color=PRIMARY, annotation_text=f"Ref V.Máx: {ref_vmax:.1f}", yref="y")
                if ref_acmax and pd.notna(ref_acmax): fig_vel.add_hline(y=ref_acmax, line_dash="dash", line_color=GOOD, annotation_text=f"Ref AC.Máx: {ref_acmax:.2f}", yref="y2")
                if ref_decmax and pd.notna(ref_decmax): fig_vel.add_hline(y=ref_decmax, line_dash="dash", line_color=WARNING, annotation_text=f"Ref DEC.Máx: {ref_decmax:.2f}", yref="y2")
    ...
    with right_col:
        ...
        if not all_dates.empty:
            ...
            # 7. Movilidad (Media de Z-Score sin lumbar)
            if df_mov is not None and not df_mov.empty:
            ...
            # 8. Dinamometría (Media de Z-Score)
            if df_dina is not None and not df_dina.empty:
                dict_pesos = {}
                if df_peso is not None and not df_peso.empty:
                    for nom_n in df_peso['Nombre_Norm'].unique():
...
with tab_gps:
    if not df_p_jug.empty:
        ...
        if not df_partidos.empty:
            best_match = df_partidos.sort_values('Dist_Total', ascending=False).iloc[0]
            m_dist_km = best_match['Dist_Total'] / 1000
            m_fecha = pd.to_datetime(best_match['Fecha_dt']).strftime('%d/%m')
            m_riv = best_match['Rival'] if best_match['Rival'] else 'Partido'
            max_dist_str = f"{m_dist_km:.1f} km ({m_riv} {m_fecha})"
        ...
        g3.metric("Media Dist/Sesión", f"{df_p_jug['Dist_Total'].mean():.0f} m" if not df_p_jug.empty else "0 m")
        g4.metric("Nº Sprints Acum.", f"{df_p_jug['N_Sprints'].sum():.0f}" if not df_p_jug.empty else "0")
        g5.metric("Acc + Dec Totales", f"{df_p_jug['Acc_Dec'].sum():.0f}" if not df_p_jug.empty else "0")
        g6.metric("Máx Demanda (Partido)", max_dist_str)
        ...
        if not df_matches_jug.empty and not df_matches_all.empty:
            ...
            with col_sel:
                selected_match_label = st.selectbox("Selecciona Partido:", options_match, index=len(options_match) - 1)
            ...
            cols_all_gps = ['Dist_Total', 'Dist_18', 'Dist_25', 'Dist_28', 'N_Sprints', 'N_Acc', 'N_Dec', 'AC_MAX', 'DEC_MAX', 'V_MAX', 'Player_Load']
            params_labels = ["Dist Total", "Dist >18", "Dist >25.2", "Dist >28.8", "Nº Sprints", "Nº Acel", "Nº Decel", "Ac Max", "Dec Max", "V Max", "Player Load"]
            ...
            if not df_matches_pos.empty:
                ...
                with col_metrics_panel:
                    ...
                    units_map = {'Dist_Total': 'm', 'Dist_18': 'm', 'Dist_25': 'm', 'Dist_28': 'm', 'N_Sprints': 'sp', 'N_Acc': 'acc', 'N_Dec': 'dec', 'AC_MAX': 'm/s²', 'DEC_MAX': 'm/s²', 'V_MAX': 'km/h', 'Player_Load': 'PL'}
                    ...
                    for i, col_k in enumerate(cols_all_gps):
                        ...
                        """, unsafe_allow_html=True)
            else: st.info("No hay datos suficientes de posición para calcular el radar.")
        else: st.info("Aún no hay registros de partidos para construir el perfil.")
        ...
        fig_gps_combined.add_trace(go.Scatter(x=df_p_jug_chart['Fecha'], y=df_p_jug_chart['Acc_Dec'], mode="lines+markers+text", name="Nº Acel + Decel", line=dict(color=GOOD, width=3), marker=dict(size=8), text=df_p_jug_chart['Label'], textposition='top center', textfont=dict(color=TEXT, size=11)), secondary_y=True)

        max_dist = (df_p_jug_chart['Dist_18'] + df_p_jug_chart['Dist_25']).max()
        max_acc_dec = df_p_jug_chart['Acc_Dec'].max()

        fig_gps_combined.update_layout(barmode="stack", height=450, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT), legend=dict(orientation="h", y=-0.2))
