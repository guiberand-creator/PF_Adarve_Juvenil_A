...
# =============================================================================
    ...
    if df_dina is None or df_dina.empty:
        ...
        # --- GRÁFICO 1: PICO DE FUERZA RELATIVO Y % ASIMETRÍA ---
        ...
        if "Rodilla" in bloque_ejercicio:
            ...
            if ratios_fe:
                ...
                fig_r_fe.add_trace(go.Bar(
                    ...
                    marker_color=['#10b981' if v_r >= 0.60 else '#ef4444' for v_r in ratios_fe],
                    text=[f"<b>{v_r:.2f}</b>" for v_r in ratios_fe], textposition='outside', textfont=dict(color='white')
                ))
            ...
            if ratios_aa:
                ...
                fig_r_aa.add_trace(go.Bar(
                    ...
                    marker_color=['#10b981' if 1.05 <= v_r <= 1.20 else '#ef4444' for v_r in ratios_aa],
                    text=[f"<b>{v_r:.2f}</b>" for v_r in ratios_aa], textposition='outside', textfont=dict(color='white')
                ))
...
# =============================================================================
    ...
    if sel_tipo_evo in ["Ambos Gráficos", "Evolución CMJ (Media de Equipo)"]:
        with col_cmj_g:
            ...
            if df_saltos is None or df_saltos.empty:
                ...
                col_t_s = 'Tipo_Norm' if 'Tipo_Norm' in df_saltos.columns else 'Tipo'
                df_cmj = df_saltos[df_saltos[col_t_s].astype(str).str.upper() == 'CMJ'].copy()
                if df_cmj.empty:
    ...
    if sel_tipo_evo in ["Ambos Gráficos", "Evolución DRI (Drop Jump 50cm)"]:
        with col_dri_g:
            ...
            if df_dri_sheet is None or df_dri_sheet.empty:
                ...
                df_dri = df_dri_sheet[df_dri_sheet['Tipo'].str.upper().str.startswith('DJ')].copy() if 'Tipo' in df_dri_sheet.columns else df_dri_sheet.copy()
                if df_dri.empty: df_dri = df_dri_sheet.copy()
    ...
    if df_saltos is None or df_saltos.empty:
        ...
        if not posiciones_s:
            ...
            # --- SECCIÓN 3: EVOLUCIÓN HISTÓRICA INTERACTIVA DE SALTOS Y DRI ---
            ...
            if tipo_grafico_salto == "Altura CMJ (cm)":
                col_t_s = 'Tipo_Norm' if 'Tipo_Norm' in df_saltos.columns else 'Tipo'
                df_cmj_all = df_saltos[df_saltos[col_t_s].astype(str).str.upper() == 'CMJ'].copy()
                if df_cmj_all.empty:
...
# =============================================================================
# ÁREA 8: RANKING GLOBAL (SIN LUMBAR)
# =============================================================================
    ...
    if df_pos is None or df_pos.empty:
        ...
        if not fechas_unicas_dt:
            ...
            if df_saltos is not None and not df_saltos.empty:
                col_tipo_s = 'Tipo' if 'Tipo' in df_saltos.columns else 'Tipo_Norm'
                df_cmj_sub = df_saltos[(df_saltos[col_tipo_s].astype(str).str.upper() == 'CMJ') & (df_saltos['Fecha_dt'] <= f_sel_dt)]
